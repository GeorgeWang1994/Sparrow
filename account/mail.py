...
# 封装EmailMultiAlternatives，实现发送HTML和txt的邮件
def send_mail(subject, message_plain, message_html, email_from, email_to,
    ...
    # thread = Thread(target=send_asyc_email, args=[msg])
    # thread.start()
    send_asyc_email(msg)
...
class UserenaConfirmationMail(object):
    ...
    def _message_in_html(self):
        if sparrow_settings.USERENA_HTML_EMAIL:
            return render_to_string(self.message_html, self.context)
        return None

    def _message_in_txt(self):
        if (not sparrow_settings.USERENA_HTML_EMAIL
            or not self.message_html
            or sparrow_settings.USERENA_USE_PLAIN_TEMPLATE):
